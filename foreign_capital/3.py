...
class a:
    def __init__(self,quantity:str,income:int,file_address:str,n:int,m:int):
        '''
        参数
    ...
    def get_num(self,delay:bool=True,continuty:bool=False)->dict:
        '''
        变量
        num_in是过去流入天数
        num_out是过去流出天数
        ...
        print('外资流入了总共%d天' % (num['num_in']))
        print('外资流出了总共%d天' % (num['num_out']))
        print('外资流入了且上涨总共%d天' % (num['num_in_up']))
        print('外资流入了且未来下跌总共%d天' % (num['num_in_down']))
        print('外资流出了且未来上涨总共%d天' % (num['num_out_up']))
        print('外资流出了且未来下跌总共%d天' % (num['num_out_down']))
        ...
        for i,j in p_dict.items():
            p_dict[i]=j*100
        print('外资流入的概率百分之', p_dict['p_in'])
        print('外资流出的概率百分之', p_dict['p_out'])
        print('在外资流入的情况下，未来上涨的概率是百分之', p_dict['p_up_in'])
        print('在外资流入的情况下，未来下跌的概率是百分之', p_dict['p_down_in'])
        print('在外资流出的情况下，未来上涨的概率是百分之', p_dict['p_up_out'])
        print('在外资流出的情况下，未来下跌的概率是百分之', p_dict['p_down_out'])
...
if __name__=='__main__':
    ...
    shili=a(quantity,income,file_address,n,m)#实例化一个类

    #使用后获取外资持股数量的基本数据，增持减持，未来的上涨和下跌天数
    b,c=shili.get_num(True,True)
